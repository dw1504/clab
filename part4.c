...
{
	pt->x = x;  //setting the values of x and y
	pt->y = y;
	//assert(0);
}
...
point_dist(struct point *pt1, struct point *pt2)
{
	...
	double distanceX = (pt1->x)-(pt2->x);  //distance between x
        double distanceY = (pt1->y)-(pt2->y);  //distance between y
        double distance = sqrt((distanceX*distanceX)+(distanceY*distanceY)); //distance 
	return distance;
	//assert(0);
	//return 0.0;
}
